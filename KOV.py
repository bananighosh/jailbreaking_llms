...
def startKOV(csvFileName, model_name, is_baseline=True):
    ...
    if is_baseline:
        results = test_transfer(fn_training_data, target_model, csvFileName, model_name, is_baseline, benchmark_indices=[0,1], temperature=0.7, verbose=True)
    else:
        results = test_transfer(fn_training_data, target_model, csvFileName, model_name, temperature=0, verbose=True)
