...
def transfer(prompt, suffixes, target_model, temperature=0, useMean=True, verbose=True, **kwargs):
    responses = []
    scores = []
    mods = []
    for i, suffix in enumerate(suffixes):
        ...
        r = score(response, use_mean=useMean, verbose = verbose, ** kwargs)
        scores.append(r)
...
def test_transfer(fn_training_data, target_model, csvFileName, model_name, is_baseline=False, n=1,
                  benchmark_indices=range(5), temperature=0, suffix_token=8, topk=32, use_mean=True, **kwargs):
    results = {}
    ...
    for benchmark_idx in benchmark_indices:
        ...
        if is_baseline:
            ...
            if not data:
                ...
                data = generate_training_data(benchmark_idx, csvFileName, model_name, suffix_token=suffix_token, top_k=topk)
            prompt = data[0]['prompt']
            suffix = extract_suffix(data)
            suffixes = [suffix] * n
        responses, scores, mods = transfer(prompt, suffixes, target_model, temperature=temperature,
                                           useMean=use_mean, ** kwargs)
...
def generate_training_data(benchmark_idx, csvFileName, model_name, suffix_token=8, top_k=32):
...
def startKOV(csvFileName, model_name, is_baseline=True, suffix_token=8, topk=32, use_mean=True, temperature=0):
    ...
    for idx, result in results.items():
        ...
        # print(f"Suffixes Length: {len(result['suffixes'])}")
        lengths_of_suffixes = [len(suffix) for suffix in result['suffixes']]
        for i, length in enumerate(lengths_of_suffixes, start=1):
